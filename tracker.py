...
if not FUND_ADDRESS:
    raise ValueError("Cannot detect address. Check .env file or upload secrets to Streamlit.")
...
def livePrice():
    print("[INFO] Requesting live price...")
...
def getTxs():
    print("[INFO] Requesting transactions...")
    ...
    for tx in data:
        ...
        print(f"[DEBUG] Working tx {txid}: blockTime={blockTime}")
...
def getPrice(blockTime):
    print(f"[INFO] Requesting price for blockTime: {blockTime}")
    ...
    try:
        ...
        print(f"[ERROR] Failed parse price response: {res.text}")
        conn.close()
        raise e
...
if __name__ == "__main__":
    try:
        ...
        # Try Requesting price for the latest 3 transactions
        for i, row in df.head(3).iterrows():
            price = getPrice(row['blockTime'])
